...
@app.middleware("http")
async def bloquear_escrita_em_demo(request: Request, call_next):
    demo_read_only = (
        os.getenv("DEMO_READ_ONLY", "false").strip().lower() == "true"
    )

    metodos_de_escrita = {"POST", "PUT", "PATCH", "DELETE"}

    if demo_read_only and request.method in metodos_de_escrita:
        return JSONResponse(
            status_code=403,
            content={
                "detail": "Ambiente de demonstração: operações de escrita estão desativadas."
            }
        )
